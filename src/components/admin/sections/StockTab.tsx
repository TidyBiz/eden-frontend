          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b-2 border-[#598C30] bg-[#C1E3A4]/50">
                <th className="text-left py-4 px-4 text-[#598C30] font-bold">
                  Producto
                </th>
                <th className="text-center py-4 px-4 text-[#598C30] font-bold">
                  PLU
                </th>
                <th className="text-right py-4 px-4 text-[#598C30] font-bold">
                  Precio
                </th>
                <th className="text-right py-4 px-4 text-[#598C30] font-bold">
                  Stock Total
            ...
            <tbody>
              {products.length > 0 ? (
                ...
                <tr>
                  <td colSpan={6} className="py-6 text-center text-[#598C30]">
                    <div className="text-center py-12 bg-[#C1E3A4]/30 rounded-xl">
          ...
          <div className="space-y-4">
            {stockAnalytics.lowStockAlerts.map((alert) => {
              ...
              return (
                <div
                  key={alert.id}
                  className="bg-[#F4F1EA66] rounded-lg p-5 transition-all duration-300 hover:shadow-lg hover:shadow-[#B0855F]/30"
                >
      ...
      <div className="bg-[#F4F1EA] rounded-2xl p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center space-x-4">
            <label className="text-[#273C1F] text-sm font-semibold">
              Filtrar por:
            </label>
            <select
              value={selectedBranchId}
              onChange={(e) => setSelectedBranchId(e.target.value)}
              className="bg-[#598C30] text-white border-2 border-[#598C30] rounded-lg px-4 py-2 text-sm transition-all font-medium"
            >
              <option value="" className='bg-white text-[#273C1F] hover:bg-[#598C30] hover:text-white cursor-pointer'>Todas las sucursales</option>
              {branches.map((branch) => (
                <option className='bg-white text-[#273C1F] hover:bg-[#598C30] hover:text-white' key={branch.id} value={branch.id}>
                  {branch.name}
                </option>
              ))}
            </select>
